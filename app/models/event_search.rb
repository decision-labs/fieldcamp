  ...
  def initialize(params={})
    ...
    if !@params[:search].blank?
      if !@params[:search][:location_ids].blank?
        ...
        all_project_ids  = Project.where{location_id.in(all_location_ids)}.select{id}
        # Project.where{parent_id.in(parent_location_ids) }.select{id} + parent_location_ids

        l = Event.where{project_id.in(all_project_ids)}
      end
      ...
      if !@params[:search][:sector_ids].blank?
        ...
        s = Event.joins{:sectors}.where{ events_sectors.sector_id.in(sectors.select{id}) }
      end
      ...
      if !@params[:search][:partner_ids].blank?
        ...
        p = Event.joins{:partners}.where{ events_partners.partner_id.in(partners.select{id}) }
      end

      # @results = Event.where{location_id.in(child_location_ids)}
